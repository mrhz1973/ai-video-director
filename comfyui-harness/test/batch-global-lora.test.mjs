...
/** Raw persisted/local Batch draft shape (pre-normalizeBatchDraft). */
function rawBatchDraft(lora = {}) {
  return {
    version: 1,
    source: sourceWithLora(lora),
    items: [
      { prompt: "job", seed: "1", duration: "5", steps: "20", megapixels: "0.4", aspect: "16:9" }
    ]
  };
}

function validateNormalizedDraft(raw) {
  const normalized = normalizeBatchDraft(raw);
  assert.ok(normalized, "normalizeBatchDraft should restore draft");
  return validateBatchOwnedLora({
    loraId: normalized.source.loraId,
    loraStrength: normalized.source.loraStrength,
...
test("normalizePersistedLoraSettings preserves explicit non-numeric strength", () => {
  assert.deepEqual(
...
test("normalizePersistedLoraSettings preserves out-of-range numeric strength", () => {
  assert.deepEqual(
    normalizePersistedLoraSettings({ loraId: "realism-people", loraStrength: 2.0 }),
    { loraId: "realism-people", loraStrength: 2.0 }
  );
  assert.deepEqual(
    normalizePersistedLoraSettings({ loraId: "realism-people", loraStrength: -0.1 }),
    { loraId: "realism-people", loraStrength: -0.1 }
  );
});

test("normalizePersistedLoraSettings omits genuinely missing strength", () => {
  assert.deepEqual(
...
test("raw draft realism-people + abc rejected after normalizeBatchDraft", () => {
  const normalized = normalizeBatchDraft(
    rawBatchDraft({ loraId: "realism-people", loraStrength: "abc" })
  );
  assert.equal(normalized.source.loraId, "realism-people");
  assert.equal(normalized.source.loraStrength, "abc");
  const result = validateNormalizedDraft(
    rawBatchDraft({ loraId: "realism-people", loraStrength: "abc" })
  );
...
test("raw draft realism-people + 2.0 rejected after normalizeBatchDraft", () => {
  const normalized = normalizeBatchDraft(
    rawBatchDraft({ loraId: "realism-people", loraStrength: 2.0 })
  );
  assert.equal(normalized.source.loraStrength, 2.0);
  const result = validateNormalizedDraft(
    rawBatchDraft({ loraId: "realism-people", loraStrength: 2.0 })
  );
  assert.equal(result.ok, false);
...
test("raw draft realism-people + -0.1 rejected after normalizeBatchDraft", () => {
  const normalized = normalizeBatchDraft(
    rawBatchDraft({ loraId: "realism-people", loraStrength: -0.1 })
  );
  assert.equal(normalized.source.loraStrength, -0.1);
  const result = validateNormalizedDraft(
    rawBatchDraft({ loraId: "realism-people", loraStrength: -0.1 })
  );
  assert.equal(result.ok, false);
...
test("raw draft realism-people + missing strength valid with default 0.70", () => {
  const normalized = normalizeBatchDraft(rawBatchDraft({ loraId: "realism-people" }));
  assert.equal(normalized.source.loraId, "realism-people");
  assert.equal(normalized.source.loraStrength, undefined);
  const result = validateNormalizedDraft(rawBatchDraft({ loraId: "realism-people" }));
  assert.equal(result.ok, true);
  assert.equal(result.selection.loraStrength, 0.7);
});

test("raw draft OFF valid after normalizeBatchDraft", () => {
  const normalized = normalizeBatchDraft(
    rawBatchDraft({ loraId: H3_LORA_OFF, loraStrength: 0.9 })
  );
  assert.equal(normalized.source.loraId, H3_LORA_OFF);
  assert.equal(normalized.source.loraStrength, undefined);
  const result = validateNormalizedDraft(rawBatchDraft({ loraId: H3_LORA_OFF }));
  assert.equal(result.ok, true);
