/**
 * Browser-safe multi-Batch queue plan constants and pure helpers.
 * ID generation is injected — see setBatchQueuePlanRandomIdFactory (server)
 * or configure in a browser entry module via crypto.randomUUID().
 */
...
/** @type {null | (() => string)} */
let defaultCreateRandomId = null;

export function setBatchQueuePlanRandomIdFactory(factory) {
  defaultCreateRandomId = typeof factory === "function" ? factory : null;
}

function cryptoRandomId(createRandomId) {
  const fn = createRandomId || defaultCreateRandomId;
  if (typeof fn !== "function") {
    throw new Error("Batch queue plan ID factory is not configured.");
  }
  return fn();
}
...
export function createQueueEntryFromDraft(draft, {
  ...
  const id = String(queueEntryId || "").trim() || cryptoRandomId(createRandomId);
...
export function normalizeQueueEntry(raw = {}, index = 0, { createRandomId = null } = {}) {
  const entry = raw && typeof raw === "object" ? raw : {};
  const snapshot = normalizeBatchDraft(entry.snapshot);
  if (!snapshot) return null;
  const queueEntryId = String(entry.queueEntryId || "").trim() || cryptoRandomId(createRandomId);
...
export function normalizeBatchQueuePlan(raw = null, { createRandomId = null } = {}) {
  if (raw == null) return null;
  const source = stripForbidden(raw);
  const entriesRaw = Array.isArray(source.entries) ? source.entries : [];
  const entries = entriesRaw
    .map((entry, index) => normalizeQueueEntry(entry, index, { createRandomId }))
    .filter(Boolean)
...
export function serializeBatchQueuePlan(plan, { bumpRevision = false, createRandomId = null } = {}) {
  const normalized = normalizeBatchQueuePlan(plan, { createRandomId });
...
export function appendQueueEntry(plan, entry, { createRandomId = null } = {}) {
  const normalized = normalizeBatchQueuePlan(plan, { createRandomId }) || {
    version: BATCH_QUEUE_PLAN_VERSION,
...
export function updateQueueEntry(plan, queueEntryId, patch = {}, { createRandomId = null } = {}) {
  const normalized = normalizeBatchQueuePlan(plan, { createRandomId });
...
export function cancelQueueEntry(plan, queueEntryId, { createRandomId = null } = {}) {
  const normalized = normalizeBatchQueuePlan(plan, { createRandomId });
  if (!normalized) return { ok: false, code: "empty-plan", error: "Coda vuota." };
  const index = normalized.entries.findIndex(entry => entry.queueEntryId === queueEntryId);
